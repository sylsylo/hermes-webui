...
function _positionModelDropdown(){
  ...
  if(!anchor) return;
  const isPhone=typeof window.matchMedia==='function'&&window.matchMedia('(max-width:710px)').matches;
  if(isPhone){
  ...
  // Desktop (>710px): keep the current master behaviour — an absolutely
...
window.addEventListener('resize',function(){
  if(window.matchMedia && !window.matchMedia('(max-width: 710px)').matches){
    closeMobileComposerConfig();
    closeModelDropdown();
    closeReasoningDropdown();
    if(typeof closeWsDropdown==='function') closeWsDropdown();
