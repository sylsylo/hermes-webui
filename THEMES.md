...
| **Icon blue** (`icon-blue`) | Dark: the Hermes app-icon palette — deep navy night surfaces (`#04162E` → `#061E3D`) with the cyan→blue emblem gradient (`#08EBF1` → `#3889FD`) as the accent family and primary buttons painted with that gradient. Light: a slate/sky palette ported from the Caducée mockup — near-white `#F8FAFC` page, WHITE surfaces separated by `#E2E8F0` hairlines and soft slate shadows (the surface ladder is deliberately *inverted* vs Default: `surface` 100 > `bg` 98.2 ≥ `sidebar`, so cards sit *above* the page, as in Catppuccin Latte), slate text `#0F172A`/`#5C6C80`, flat `#0284C7` primary buttons and `#E0F2FE`/`#BAE6FD` pill badges. The mockup's sky-600 is kept off text (it only reaches 4.10:1): `--accent`/`--accent-text` stay sky-700 `#0369A1` (5.7:1 as text, 5.9:1 for white on the user bubble), so every text token clears WCAG AA. |
